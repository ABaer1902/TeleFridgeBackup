...
if ($sortByFridge && $id != NULL) {
    ...
    $sql = "SELECT fridge FROM accounts WHERE id = ? AND fridge IS NOT NULL";
    ...
    a.id AS creator_id,
    (r.likes - COALESCE(r.dislikes, 0)) AS net_likes
    FROM 
    recipes AS r
    INNER JOIN 
    accounts AS a ON r.creator_id = a.id
    INNER JOIN
    followers AS f ON a.id = f.following_id
    WHERE 
    f.follower_id = $id
    ORDER BY 
    net_likes DESC";
