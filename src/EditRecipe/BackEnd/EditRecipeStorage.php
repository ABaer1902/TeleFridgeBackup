...
//backend php functions for recipe editing

//read from query string and verify user [REQUIRES QUERY VAR]
function verifyUser($user_id) {
    //add query name and make sure to check if set
    $recipe_id = ""; //$_GET['']
    ...
    //query
    $cookieQuery = "SELECT creator_id FROM recipes WHERE id=?";
    ...
    if ($user_id != $creator_id) {
        echo "Unauthorized: You May Not Edit This Recipe";
        exit;
    } else {
        return true;
    }
...
function deleteRecipe($recipe_id) {
    ...
    //query
    $cookieQuery = "DELETE FROM recipes WHERE id=?";
    $stmt = $conn->prepare($cookieQuery);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES['recipe-pic']['size'] == 0){
        $oldpic = $_POST['old-pic'];
        $picture = base64_decode($oldpic);
    }
    else{
    ...
    $sql = "UPDATE recipes SET name=?, difficulty=?, hours=?, minutes=?, description=?, instructions=?, ingredients=?, picture=?, calories=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('siiissssii', $recipe, $difficulty, $hours, $minutes, $description, $instructions, $ingredientsList,  $picture, $calories, $_GET["recipe_id"]);
    ...
    // Close connection
    $conn->close();
    $url = (empty($_SERVER['HTTPS']) ? 'http' : 'https') . "://$_SERVER[HTTP_HOST]" . strtok($_SERVER["REQUEST_URI"], '?') . "/../../Profile/Profile.php";
    header('Location: '.$url);
